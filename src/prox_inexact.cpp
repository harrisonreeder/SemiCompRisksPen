...
arma::vec soft_thresh(const arma::vec& x, const double y) {
  ...
  arma::vec z = arma::zeros(x.n_elem);
...
// [[Rcpp::export]]
arma::vec admm_po_cpp(const arma::vec& beta_tilde, const arma::vec& slambda, 
  ...
  arma::vec xhat = arma::zeros(d);
  arma::vec z_old = arma::zeros(m);
  ...
  if (fast) {
    ...
    ADMM_aux = arma::eye(d, d) - rho * Q * diagmat(1/(1/eigval + rho)) * Qt;
    
  } else {
    // Slower (standard) version to compute inverse
    ADMM_aux = inv(arma::eye(d, d) + rho * penmat_t * penmat);
  }
